...
async fn test_multipart_upload_integration() -> Result<(), ObsError> {
    ...
    assert_eq!(
        &body_ref[0..part1_data.len()],
        part1_data.as_slice(),
        "第1段内容应该匹配"
    );
    assert_eq!(
        &body_ref[part1_data.len()..part1_data.len() + part2_data.len()],
        part2_data.as_slice(),
        "第2段内容应该匹配"
    );
    assert_eq!(
        &body_ref[part1_data.len() + part2_data.len()..],
        part3_data.as_slice(),
        "第3段内容应该匹配"
    );
...
async fn test_multipart_upload_with_completed_parts() -> Result<(), ObsError> {
    ...
    for part_num in 1..=3 {
        let part_data: Vec<u8> = (0..10 * 1024)
            .map(|i| ((i + part_num) % 256) as u8)
            .collect();
